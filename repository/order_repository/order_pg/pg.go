...
func (orderPG *orderPG) ReadOrderById(orderId int) (*entity.Order, errs.Error) {
	...
	row := orderPG.db.QueryRow(getOrderByIdQuery, orderId)
...
func (orderPG *orderPG) ReadOrders() ([]order_repository.OrderItemMapped, errs.Error) {
	row, err := orderPG.db.Query(getOrdersWithItemsQuery)
	...
	for row.Next() {
		...
		err := row.Scan(&orderItem.Order.OrderId, &orderItem.Order.CustomerName, &orderItem.Order.OrderedAt, &orderItem.Order.CreatedAt, &orderItem.Order.UpdatedAt,
...
func (orderPG *orderPG) CreateOrder(orderPayLoad entity.Order, itemPayLoad []entity.Item) errs.Error {
	// proses membuat data order dan item
	tx, err := orderPG.db.Begin()
